...
# # # # # # # # # # # # # # # # # # # # # #
# Running the ARIMA model = usdO - 1 year #
# # # # # # # # # # # # # # # # # # # # # #
...
usd_arima_1year = pd.read_csv("./datasets/arima_ready/usd_arima_1year.csv", float_precision="high", parse_dates=([0]))

df_usd_arima_train = usd_arima_1year

df_usd_arima_train.info()

df_usd_arima_test = pd.read_csv("./datasets/arima_ready/usd_arima_3months.csv", float_precision="high", parse_dates=([0]))

df_usd_arima_test.info()

y = df_usd_arima_train["diff"]

test_diff = df_usd_arima_test["log"]

# In[1.0]: Training the model SARIMAX(0,0,0)

usd_arima_final_002 = SARIMAX(y, exog=None, order=(0,0,2), seasonal_order=(0,0,0,0), enforce_stationarity=True, trend="c").fit()

usd_arima_final_002.summary()

"""
                               SARIMAX Results                                
==============================================================================
Dep. Variable:                   diff   No. Observations:                  280
Model:               SARIMAX(0, 0, 2)   Log Likelihood                1065.731
Date:                Sun, 29 Sep 2024   AIC                          -2123.463
Time:                        13:22:43   BIC                          -2108.923
Sample:                             0   HQIC                         -2117.631
                                - 280                                         
Covariance Type:                  opg                                         
==============================================================================
...
ma.L1         -0.0469      0.059     -0.788      0.431      -0.163       0.070
ma.L2         -0.1682      0.061     -2.752      0.006      -0.288      -0.048
sigma2      2.813e-05   1.79e-06     15.698      0.000    2.46e-05    3.16e-05
===================================================================================
Ljung-Box (L1) (Q):                   0.00   Jarque-Bera (JB):                51.49
Prob(Q):                              0.94   Prob(JB):                         0.00
Heteroskedasticity (H):               1.09   Skew:                             0.41
Prob(H) (two-sided):                  0.67   Kurtosis:                         4.93
===================================================================================
...
usd_arima_final_002.plot_diagnostics(figsize=(10,7))
plt.show()
...
df_usd_arima_train["erros000"] = usd_arima_final_000.resid

erros000 = df_usd_arima_train["erros000"].dropna()
...
sns.scatterplot(x=df_usd_arima_train["date"], y=erros000, color="green", alpha=0.4,
             edgecolor=None)
...
usd_arima_test_final = usd_arima_final_002int.apply(df_usd_arima_test["diff"])



# In[]: Testing the fitted values

df_usd_arima_train["yhat"] = usd_arima_final_002int.fittedvalues
usd_arima_fitted = pd.DataFrame(usd_arima_final_002int.fittedvalues)

# Fazendo previsões:

usd_arima_002_predict = usd_arima_final_002int.predict(0, 14 )

predicted = invert_results(usd_arima_002_predict, usd_arima_1year.iloc[5,2])
...
plt.figure(figsize=(15, 10))
sns.scatterplot(x=df_usd_arima_train["date"], y=df_usd_arima_train["erros"], color="green", alpha=0.4,
             edgecolor=None)
plt.title("Resíduos do Modelo - ARIMA(0,0,2) + intercepto", fontsize="18")
plt.xlabel("")
plt.ylabel("")
